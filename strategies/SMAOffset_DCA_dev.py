...
class SMAOffset_DCA_dev(IStrategy):
    ...
    base_nb_candles_buy = IntParameter(
        5, 80, default=16, space='buy', optimize=True)
    base_nb_candles_sell = IntParameter(
        5, 80, default=20, space='sell', optimize=True)
    low_offset = DecimalParameter(
        0.9, 0.99, default= 0.973, space='buy', optimize=True)
    high_offset = DecimalParameter(
        0.99, 1.1, default=1.010, space='sell', optimize=True)

    # Protection
    fast_ewo = 50
    slow_ewo = 200
    ewo_low = DecimalParameter(-20.0, -8.0,
                               default=-19.931, space='buy', optimize=True)
    ewo_high = DecimalParameter(
        2.0, 12.0, default=5.672, space='buy', optimize=True)
